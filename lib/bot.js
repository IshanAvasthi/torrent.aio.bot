...
const downloadRegex = /\/torrent (.+)/;
const statusRegex = /\/status (.+)/;
const removeRegex = /\/remove (.+)/;

const startMessage = `*Hello there ${msg.chat.first_name}*, This is Torrenter.DC, a torrenter bot made using [torrent-aio-bot repository](https://github.com/patheticGeek/torrent-aio-bot) by [patheticGeek](https://github.com/patheticGeek). Created, edited and maintained by @dcbots and @dcdunkan.
*Search* torrents, find *details* of torrent, *Download* torrent to Google Drive, *Upload* telegram files to Google Drive. These tasks can be easily done with this Bot. But *Remember, this bot is completely private.*
Currently, Piratebay, 1337x and Limetorrent are available to search for a Torrent file.
Click on *How to create* to learn how to create a bot just like this. Also, you can read other documentations too. Join @dcbots for more bots just like this.
`;

function bot(torrent, bot) {
  bot.onText(/\/start/, async msg => {
    bot.sendMessage(msg.chat.id, startMessage, {parse_mode: "Markdown", {reply-markup: {inline-keyboard : [[{text:"How to create?", url: "https://github.com/patheticGeek/torrent-aio-bot"}]]}}});
  });
